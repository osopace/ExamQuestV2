...
export default function CoursesPage() {
  const progressMap = Object.fromEntries(COURSE_PROGRESS.map((p) => [p.course_id, p]));
  const enrolled = COURSES.filter((c) => COURSE_PROGRESS.some((p) => p.course_id === c.id));
  const browse = COURSES.filter((c) => !COURSE_PROGRESS.some((p) => p.course_id === c.id));
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-8">
        ...
        <section>
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="text-sm text-gray-500">{enrolled.length} courses</span>
          </div>
                ...
                <Card key={course.id} padding="none" className="overflow-hidden">
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0" style={{ backgroundColor: course.color + "15" }}>📚</div>
                      <Badge variant={progress?.progress_percent >= 70 ? "success" : "primary"}>
                        {progress?.progress_percent >= 100 ? "Completed" : "Active"}
                      </Badge>
                    </div>
                    ...
                    <ProgressBar value={progress?.progress_percent ?? 0} size="sm" className="mb-3" showLabel />
                    <div className="flex items-center justify-between text-xs text-gray-500 mb-4">
                      <span>{progress?.total_quizzes ?? 0} quizzes taken</span>
                      {progress?.last_practiced_at && (
                        <span className="flex items-center gap-1"><Clock size={11} />{formatRelativeDate(progress.last_practiced_at)}</span>
                      )}
                    </div>
                    <div className="flex gap-2">
                      <Link href={`/practice?course=${course.id}`} className="flex-1">
        ...
        {browse.length > 0 && (
          <section>
            <h2 className="text-xl font-bold text-gray-900 mb-5">Browse More Courses</h2>
            <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {browse.map((c) => (
                <Card key={c.id} hoverable padding="md" className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: c.color + "20" }}>📚</div>
