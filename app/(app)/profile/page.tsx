...
import { Camera, Save, Flame, Trophy, BookOpen, Target, Mail, Eye, EyeOff, Loader2 } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    if (!profile) return;
    if (!fullName.trim()) { setNameError("Full name cannot be empty"); return; }
    setNameError("");
    setLoading(true);
    try {
        ...
        <Card padding="lg" className="flex flex-col sm:flex-row items-center gap-6">
          <div className="relative">
            <div className={cn("relative", avatarUploading && "opacity-60")}>
              <Avatar name={profile?.full_name ?? "?"} src={profile?.avatar_url ?? undefined} size="xl" />
              {avatarUploading && (
            ...
            <div className="flex items-center gap-2 mt-2 justify-center sm:justify-start flex-wrap">
              ...
              {savedExamTypes.map((et, i) => (
                <Badge key={et} variant={i === 0 ? "info" : "neutral"}>
                  {EXAM_OPTIONS.find((o) => o.id === et)?.label ?? et.toUpperCase()}
                  {i === 0 && savedExamTypes.length > 1 && <span className="opacity-60 ml-0.5">·primary</span>}
                </Badge>
              ))}
            </div>
          </div>
        ...
        <Card padding="md">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1.5">A confirmation link will be sent to your new address.</p>
            </div>

            {/* Exam type chip grid */}
            <div>
              <div className="flex items-center justify-between mb-3">
                <label className="text-sm font-medium text-gray-700">Active Exam Types</label>
                <span className={cn(
                  "text-xs font-semibold px-2.5 py-0.5 rounded-full transition-colors",
                  examTypes.length >= 2 ? "bg-primary-100 text-primary-700" : "bg-gray-100 text-gray-500"
                )}>
                  {examTypes.length} / 2
                </span>
              </div>
                ...
                {EXAM_OPTIONS.map(({ id, label, sub }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={cn("font-bold text-sm", active ? "text-primary-700" : "text-gray-700")}>
                        {label}
                      </span>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-500 leading-relaxed">
            Verify your identity and enter your new email. Supabase will send a confirmation link to your new address — the change takes effect once you click it.
          </p>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <Button fullWidth onClick={handleEmailChange} loading={emailLoading} leftIcon={<Mail size={15} />}>
              Send Confirmation
            </Button>
          </div>
        </div>
