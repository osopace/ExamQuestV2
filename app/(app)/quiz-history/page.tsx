...
import { BookOpen, Clock, CheckCircle, XCircle, Filter } from "lucide-react";
import { Card, Badge } from "@/components/ui/index";
import Topbar from "@/components/shared/Topbar";
import { MOCK_QUIZ_HISTORY } from "@/constants/mockData";
import { formatDate, formatDuration, getScoreBg } from "@/utils/format";

type Filter = "all" | "quiz" | "study";

export default function QuizHistoryPage() {
  const [filter, setFilter] = useState<Filter>("all");

  const filtered = MOCK_QUIZ_HISTORY.filter((q) => filter === "all" || q.mode === filter);

  const totalQuizzes = MOCK_QUIZ_HISTORY.length;
  const avgScore = Math.round(MOCK_QUIZ_HISTORY.reduce((s, q) => s + (q.score_percent ?? 0), 0) / totalQuizzes);
  const bestScore = Math.max(...MOCK_QUIZ_HISTORY.map((q) => q.score_percent ?? 0));
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Average Score", value: `${avgScore}%`, icon: CheckCircle, color: "text-green-600", bg: "bg-green-50" },
            { label: "Best Score", value: `${bestScore}%`, icon: CheckCircle, color: "text-amber-600", bg: "bg-amber-50" },
        ...
        <div className="flex gap-2">
          {(["all", "quiz", "study"] as Filter[]).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
