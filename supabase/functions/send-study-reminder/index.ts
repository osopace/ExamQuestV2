  ...
  return `
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;">
      ...
      <div style="padding:40px;">
        <h2 style="color:#111827;margin-top:0;font-size:20px;">Study time, ${name}! 📖</h2>
        <p style="color:#6b7280;font-size:16px;line-height:1.6;margin:0 0 24px;">
          Consistent daily study sessions are proven to outperform cramming. Even 20 minutes today makes a real difference on exam day.
        </p>
        <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:20px;margin:0 0 28px;">
          <p style="margin:0;color:#166534;font-size:15px;font-weight:600;">Today's study tip</p>
          <p style="margin:8px 0 0;color:#15803d;font-size:14px;line-height:1.6;">
            Focus on one subject at a time. Review your weakest topics first while your mind is fresh, then reinforce your stronger areas.
          </p>
        </div>
        <div style="text-align:center;margin:0 0 32px;">
          <a href="${APP_URL}/courses" style="background:#6366f1;color:white;padding:14px 36px;border-radius:8px;text-decoration:none;font-weight:600;font-size:16px;display:inline-block;">Go to My Courses →</a>
        </div>
        <hr style="border:none;border-top:1px solid #f3f4f6;margin:0 0 20px;" />
        <p style="color:#9ca3af;font-size:13px;margin:0;text-align:center;">
          You're receiving this because you have study reminders enabled.<br/>
...
async function sendEmail(to: string, name: string): Promise<boolean> {
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: FROM_EMAIL,
      to: TEST_EMAIL ?? to,
      subject: `Time to study, ${name}! Your exam won't prep itself 💪`,
    ...
    for (const profile of profiles ?? []) {
      ...
      if (userSetting?.push_notifications) {
        await supabase.from("notifications").insert({
          ...
          type: "study_reminder",
          title: "Study Time 📖",
          body: "Consistent daily study beats cramming. Even 20 minutes today makes a real difference on exam day.",
        });
      }
    }
